...
def compute_all_descriptors(smiles_list, n_jobs=None):
    ...
    Args:
        ...
        n_jobs: Number of parallel jobs. If None, uses all available CPUs minus 4
...
# Helper to append descriptors to a dataframe with 'smiles'
...
# Argument parsing - Modified for BASE/FPS/Hablation comparison
parser = argparse.ArgumentParser(description='Compare BASE and FPS models against time hablation model.')
parser.add_argument('-ori', choices=['nosotros', 'guacamol', 'pubchem'], required=True, help='Select original data source: nosotros|guacamol|pubchem')
parser.add_argument('-dir', '--directory', nargs=2, required=True, help='Two directories: BASE model directory and FPS model directory')
parser.add_argument('-hablation', action='store_true', required=True, help='Include Time hablation model in comparison')
parser.add_argument('-ref', '--reference', type=str, default='ori', help='The reference category for comparison (default: ori)')
...
parser.add_argument('-n_jobs', '--n_jobs', type=int, default=None, help='Number of CPU cores to use for parallel processing (default: all available cores minus 4)')
...
# Check that exactly 2 directories are provided
if len(args.directory) != 2:
    raise ValueError("You must provide exactly 2 directories: BASE model and FPS model.")

directories = args.directory

# Set generated file name
generated_filename = 'all_generated_molecules.csv'
graph_dir_name = 'graficas_random_10K_positive_approach'

# Add suffix for pubchem if that option is selected
if args.ori == 'pubchem':
    graph_dir_name += '_pubchem'

# Set up the 4 categories: ori, BASE, FPS, and hablation
selected_categories = [
    'ori',
    'gen_' + directories[0],  # BASE model
    'gen_' + directories[1],  # FPS model
    'hablation'
]
...
if args.load_descriptors:
    ...
    # Read CSV files for BASE and FPS models
    df_base = pd.read_csv(f'../mols_gen/{directories[0]}/{generated_filename}')
    df_fps = pd.read_csv(f'../mols_gen/{directories[1]}/{generated_filename}')
    ...
    # Select up to 100000 random molecules and apply <70 atoms filter
    ...
    # Select 15000 random molecules from BASE model and filter by <70 atoms
    selected_smiles_base = df_base.sample(n=15000, random_state=1111)['smiles'].tolist()
    selected_smiles_base = filter_smiles_lt70(selected_smiles_base)
    
    # Select 15000 random molecules from FPS model and filter by <70 atoms
    selected_smiles_fps = df_fps.sample(n=15000, random_state=1111)['smiles'].tolist()
    selected_smiles_fps = filter_smiles_lt70(selected_smiles_fps)

    # Separate the dataframe into original, BASE, FPS
    df_ori = pd.DataFrame({'smiles_ori': selected_smiles})
    df_gen_base = pd.DataFrame({'smiles_gen': selected_smiles_base})
    df_gen_fps = pd.DataFrame({'smiles_gen': selected_smiles_fps})
    
    # Load hablation model
    ...
    if args.hablation and os.path.exists(hablation_path):
        ...
        raise ValueError(f"Hablation model not found at {hablation_path}")

    # Rename columns to have the same name across dataframes
    df_ori = df_ori.rename(columns={'smiles_ori': 'smiles'})
    df_gen_base = df_gen_base.rename(columns={'smiles_gen': 'smiles'})
    df_gen_fps = df_gen_fps.rename(columns={'smiles_gen': 'smiles'})

    # Apply <70 atoms filter to all dataframes and drop invalid
    df_ori = filter_df_smiles_lt70(df_ori, 'smiles')
    df_gen_base = filter_df_smiles_lt70(df_gen_base, 'smiles')
    df_gen_fps = filter_df_smiles_lt70(df_gen_fps, 'smiles')

    # Add a column to indicate the category
    df_ori['category'] = 'ori'
    df_gen_base['category'] = 'gen_' + directories[0]
    df_gen_fps['category'] = 'gen_' + directories[1]

    # Reset index for each DataFrame
    df_ori = df_ori.reset_index(drop=True)
    df_gen_base = df_gen_base.reset_index(drop=True)
    df_gen_fps = df_gen_fps.reset_index(drop=True)

    # Check if df_ori contains column time_pred
    if 'time_pred' in df_ori.columns:
        df_ori = df_ori.drop(columns=['time_pred'])
    if 'time_pred' in df_gen_base.columns:
        df_gen_base = df_gen_base.drop(columns=['time_pred'])
    if 'time_pred' in df_gen_fps.columns:
        df_gen_fps = df_gen_fps.drop(columns=['time_pred'])

    # Compute descriptors for each category DataFrame
    df_ori = add_descriptors(df_ori, 'smiles', n_jobs=args.n_jobs)
    df_gen_base = add_descriptors(df_gen_base, 'smiles', n_jobs=args.n_jobs)
    df_gen_fps = add_descriptors(df_gen_fps, 'smiles', n_jobs=args.n_jobs)
    df_hablation = add_descriptors(df_hablation, 'smiles', n_jobs=args.n_jobs)

    # Concatenate all DataFrames
    frames = [df_ori, df_gen_base, df_gen_fps, df_hablation]
...
# Ensure the directory exists and save the Jensen-Shannon distances
...
def plot_feature_distributions_with_js(dataframe, features, js_distances, bins=30):
    ...
    # We have 2 comparisons: BASE vs hablation and FPS vs hablation
    comparison_models = ['BASE', 'FPS']  # These are labels for the two CoCoGraph models
    num_comparisons = 2
    
    # Create separate axes for each comparison in the right column of rows 2 and 3
    # Split the space spanning rows 1:3, column 3 into num_comparisons subplots
    gs_right = gridspec.GridSpecFromSubplotSpec(num_comparisons, 1, subplot_spec=gs[1:3, 3], hspace=0.3)
    comparison_axes = []
    ...
    for i, (ax, feature) in enumerate(zip(distribution_axes, selected_features)):
        ...
        if feature in DISCRETE:
            ...
            plot_order = ['gen_' + directories[0], 'gen_' + directories[1], 'hablation', 'ori']
                ...
                if len(data) > 0:
                    ...
                    if cat == reference_category:
                        ...
                        if cat == 'gen_' + directories[0]:
                            label = f"CoCoGraph BASE"
                        elif cat == 'gen_' + directories[1]:
                            label = f"CoCoGraph FPS"
                        elif cat == 'hablation':
                    ...
                    if cat != reference_category and len(js_values) > 0:
                        ...
                        # Order from top to bottom: CoCoGraph BASE (+0.15), CoCoGraph FPS (0.0), Time hablation (-0.15)
                        if i == 2:  # Plot C (3rd plot)
                            ax.text(0.95, 0.7 + (0.15 * (cat == 'gen_' + directories[0])) + (0.0 * (cat == 'gen_' + directories[1])) - (0.15 * (cat == 'hablation')), 
                            ...
                            ax.text(0.1, 0.8 + (0.15 * (cat == 'gen_' + directories[0])) + (0.0 * (cat == 'gen_' + directories[1])) - (0.15 * (cat == 'hablation')), 
                                    js_text, transform=ax.transAxes, fontsize=22, weight='bold',
                            ...
                            ax.text(0.95, 0.3 + (0.15 * (cat == 'gen_' + directories[0])) + (0.0 * (cat == 'gen_' + directories[1])) - (0.15 * (cat == 'hablation')), 
                                    js_text, transform=ax.transAxes, fontsize=22, weight='bold',
                                    color=style['color'], ha='right', va='center', fontname='Nimbus Sans')
            ...
            for cat in plot_order:
                ...
                if cat != reference_category and len(js_values) > 0:
                    ...
                    if i == 2:  # Plot C (3rd plot)
                        ...
                        ax.text(0.1, 0.7 + (0.15 * (cat == 'gen_' + directories[0])) + (0.0 * (cat == 'gen_' + directories[1])) - (0.15 * (cat == 'hablation')), 
                                js_text, transform=ax.transAxes, fontsize=22, weight='bold',
                                color=style['color'], ha='left', va='center', fontname='Nimbus Sans')
                    else:
                        # Position in the right middle area, with slight vertical offsets to prevent overlap
    ...
    if len(distribution_axes) > 0:
        ...
        if handles:
            first_ax.legend(handles, labels, fontsize=22, loc='upper left', bbox_to_anchor=(0.2, 1.0))

    # Now, create separate log JS ratios plots for each comparison model
    pd.set_option('display.max_columns', None)
    pd.reset_option('display.max_columns')
    
    # Calculate JS ratios for each comparison: BASE vs hablation and FPS vs hablation
    js_ratios_dict = {}
    better_features_dict = {}
    worse_features_dict = {}
    
    # For BASE model vs hablation
    js_ratios_base = []
    for feat in features:
        js_base_model = js_distances[
            (js_distances['Feature'] == feat) &
            (js_distances['Category1'] == 'ori') &
            (js_distances['Category2'] == 'gen_' + directories[0])
        ]['JS_Distance'].values
        
        js_hablation = js_distances[
            (js_distances['Feature'] == feat) &
            (js_distances['Category1'] == 'ori') &
            (js_distances['Category2'] == 'hablation')
        ]['JS_Distance'].values
        
        if len(js_base_model) > 0 and len(js_hablation) > 0:
            js_base_model = np.where(js_base_model == 0, 0.0001, js_base_model)
            js_hablation = np.where(js_hablation == 0, 0.0001, js_hablation)
            js_ratio = np.log2(js_hablation / js_base_model)
            js_ratios_base.extend(js_ratio)
    
    js_ratios_dict['BASE'] = js_ratios_base
    better_features_dict['BASE'] = int(np.sum(np.array(js_ratios_base) > 0))
    worse_features_dict['BASE'] = int(np.sum(np.array(js_ratios_base) < 0))
    
    # For FPS model vs hablation
    js_ratios_fps = []
    for feat in features:
        js_fps_model = js_distances[
            (js_distances['Feature'] == feat) &
            (js_distances['Category1'] == 'ori') &
            (js_distances['Category2'] == 'gen_' + directories[1])
        ]['JS_Distance'].values
        ...
        if len(js_fps_model) > 0 and len(js_hablation) > 0:
            js_fps_model = np.where(js_fps_model == 0, 0.0001, js_fps_model)
            js_hablation = np.where(js_hablation == 0, 0.0001, js_hablation)
            js_ratio = np.log2(js_hablation / js_fps_model)
            js_ratios_fps.extend(js_ratio)
    
    js_ratios_dict['FPS'] = js_ratios_fps
    better_features_dict['FPS'] = int(np.sum(np.array(js_ratios_fps) > 0))
    worse_features_dict['FPS'] = int(np.sum(np.array(js_ratios_fps) < 0))
    ...
    model_colors = {
        'BASE': category_styles['gen_' + directories[0]]['color'],
        'FPS': category_styles['gen_' + directories[1]]['color']
    }
    ...
    for idx, (model, ax_log) in enumerate(zip(comparison_models, comparison_axes)):
        ...
        ax_log.text(3, y_limit * 0.70, f"{better_features_dict[model]}", 
                    color=model_colors[model], fontsize=20, 
                    ha='center', weight='bold', fontname='Nimbus Sans', zorder=10, alpha=1.0)
        ...
        # Add "CoCoGraph BASE" or "CoCoGraph FPS" with appropriate color
        model_name = f'CoCoGraph {model}'
        ax_log.text(0.35, title_y_pos, model_name, transform=ax_log.transAxes, 
                   fontsize=24, weight='bold', fontname='Nimbus Sans', 
                   ha='right', va='bottom', color=model_colors[model])
        # Add "vs" in black in the center
        ax_log.text(0.5, title_y_pos, 'vs', transform=ax_log.transAxes, 
                   fontsize=24, weight='bold', fontname='Nimbus Sans', 
                   ha='center', va='bottom', color='black')
        # Add "Time hablation" in its color on the right
        ax_log.text(0.65, title_y_pos, f'{model_display_names[model]}', transform=ax_log.transAxes, 
                   fontsize=24, weight='bold', fontname='Nimbus Sans', 
                   ha='left', va='bottom', color=category_styles['hablation']['color'])
...
for feat in columns_to_compare:
    # Get JS distance for BASE model vs original
    js_base_model = js_distances_kde[
        (js_distances_kde['Feature'] == feat) &
        (js_distances_kde['Category1'] == 'ori') &
        (js_distances_kde['Category2'] == 'gen_' + directories[0])
    ]['JS_Distance'].values
    
    # Get JS distance for FPS model vs original
    js_fps_model = js_distances_kde[
        (js_distances_kde['Feature'] == feat) &
        (js_distances_kde['Category1'] == 'ori') &
        (js_distances_kde['Category2'] == 'gen_' + directories[1])
    ]['JS_Distance'].values
    
    # Get JS distance for hablation vs original
    js_hablation = js_distances_kde[
        (js_distances_kde['Feature'] == feat) &
        (js_distances_kde['Category1'] == 'ori') &
        (js_distances_kde['Category2'] == 'hablation')
    ]['JS_Distance'].values
    
    # Extract values
    js_base = js_base_model[0] if len(js_base_model) > 0 else np.nan
    js_fps = js_fps_model[0] if len(js_fps_model) > 0 else np.nan
    js_hab = js_hablation[0] if len(js_hablation) > 0 else np.nan
    ...
    # Compute log2 ratios (hablation/BASE and hablation/FPS)
    log2_base_vs_hablation = np.log2(js_hab / js_base) if (not np.isnan(js_base) and not np.isnan(js_hab)) else np.nan
    log2_fps_vs_hablation = np.log2(js_hab / js_fps) if (not np.isnan(js_fps) and not np.isnan(js_hab)) else np.nan

    result_list.append({
        'Descriptor': feat,
        'Tier': tier_dict.get(feat, 'Unknown'),
        'log2_BASE_vs_hablation': log2_base_vs_hablation,
        'log2_FPS_vs_hablation': log2_fps_vs_hablation
    })
