            ...
            for cat in plot_order:
                ...
                if len(data) > 0:
                    ...
                    if cat != reference_category and len(js_values) > 0:
                        ...
                        # Order from top to bottom: CoCoGraph, Digress, JTVAE, DeFog strict, DeFog relaxed, Time hablation
                        ax.text(0.95, js_y_base + (0.15 * (cat == 'gen_' + directories[0])) + (0.075 * (cat == 'digress')) + (0.0 * (cat == 'jtvae')) - (0.075 * (cat == 'defog_strict')) - (0.15 * (cat == 'defog_relaxed')) - (0.225 * (cat == 'hablation')), 
