...
graph_dir_name = 'graficas_guacamol_random_10K_composite'
            ...
            for cat in plot_order:
                ...
                if len(data) > 0:
                    ...
                    if cat != reference_category and len(js_values) > 0:
                        ...
                        # Order from top to bottom: CoCoGraph BASE, CoCoGraph FPS, Time hablation
                        ax.text(0.95, js_y_base + (0.15 * (cat == 'gen_' + directories[0])) + (0.0 * (cat == 'gen_' + directories[1])) - (0.15 * (cat == 'hablation')), 
...
for feat in columns_to_compare:
    ...
    # Calculate log2 ratios (hablation/BASE and hablation/FPS)
    log2_base_vs_hablation = np.nan
    log2_fps_vs_hablation = np.nan
    
    if len(js_base_model) > 0 and js_base_model[0] != 0 and len(js_hablation) > 0 and js_hablation[0] != 0:
        log2_base_vs_hablation = np.log2(js_hablation[0] / js_base_model[0])
    
    if len(js_fps_model) > 0 and js_fps_model[0] != 0 and len(js_hablation) > 0 and js_hablation[0] != 0:
        log2_fps_vs_hablation = np.log2(js_hablation[0] / js_fps_model[0])
