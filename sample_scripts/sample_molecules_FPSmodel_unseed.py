...
def process_batch(conjunto, model, num, b_molecule, cantidad, time_model, future_timeout_seconds=300):
    ...
    # Load valence/charge/radical configurations (once per batch)
    ...
    for cuenta_smiles, sigma_i in enumerate(sigma_list):
        ...
        # Build a noisy graph directly from molecular formula with retries (no random swaps here)
        smi = smiles[count]
        try:
            mol_from_smiles = Chem.MolFromSmiles(smi)
            formula = Chem.rdMolDescriptors.CalcMolFormula(mol_from_smiles) if mol_from_smiles is not None else ''
            # get number of atoms and implicit hydrogens
            
        except Exception:
            formula = ''
            error += 1

        g_orig = smiles_to_graph(smi)
        num_atoms = g_orig.number_of_nodes()
        print(f"Number of atoms original: {num_atoms}")
        if not isinstance(formula, str) or not formula:
            print(f"[process_batch] Skipping molecule at idx {count}: cannot derive formula from SMILES '{smi}'")
            count = count + 1
            error += 1
            continue
        ...
        g_ruido = None
        try:
            #print(f"Valence weights: {valence_weights}")
            #print(f"Charge symbol weights: {charge_symbol_weights}")
            #print(f"Allow radicals: {radical_symbol_weights is not None}")
            #print(f"Radical symbol weights: {radical_symbol_weights}")
            G = build_gt_from_formula(
                formula,
                randomize_swaps=0,
            ...
            g_ruido = G
            num_atoms = g_ruido.number_of_nodes()
            print(f"Number of atoms noisy: {num_atoms}")
        except Exception as e:
            print(f"Error building noisy graph from formula '{formula}' after 100 attempts (idx {count}): {e}")
            error += 1
            continue

        if g_ruido is None:
            print(f"[process_batch] Omitted formula '{formula}' after 100 attempts (idx {count}).")
            count = count + 1
            error += 1
            continue

        # Determine the number of swaps based on the noisy graph edges
        try:
            edges_noisy = g_ruido.number_of_edges()
            print(f"Number of edges in noisy graph: {edges_noisy}")
            print(f"Number of edges in valid graph: {torch.sum(valid_edge_b[count]).item() // 2 }")
        except Exception:
            error += 1
            edges_noisy = int(torch.sum(valid_edge_b[count]).item() // 2)
            print(f"Error calculating number of edges in noisy graph, so used the original number of edges")
       
        try:
            num_cambios = math.ceil(sigma_i * edges_noisy)
            ...
            noisy_graphs.append(g_ruido)
            count=count+1
        except Exception:
            count +=1
            error += 1
            print(f"Error embedding edges, so skipped this molecule")

        # ahora mismo si error ha saltado, no vamos a la siguiente molecula
        # aun asi no importa porque se salta el minibatch de 50 entero... 
        # pero, ¿porque hay mleculas que tienen un nodo mas cuando las crea el noisy que cuando pasan por el dataloader? 

    if error > 0:
        print(f"Error: {error} molecules skipped")
    ...
    while molecule_counter < num_swaps_max:
        ...
        # el error ha sido aqui, dentro al hacer update property cache, por una explicit valence de un N de 4. 
        try:
            futures = [executor_gpu.submit(calculate_data_molecule_fps, graph, tensor, num_swaps, molecule_counter) for graph, tensor, num_swaps in zip(noisy_graphs, tensors_allmolecules, num_swaps)]

            # Wait up to the configured timeout for all futures to complete
            done, not_done = wait(futures, timeout=future_timeout_seconds)
            if len(not_done) > 0:
                for f in not_done:
                    f.cancel()
                raise TimeoutError(f"Timed out after {future_timeout_seconds}s waiting for calculate_data_molecule_fps; {len(not_done)} incomplete")

            # Get the results (all are done here)
            results = [future.result() for future in futures]
        except Exception as e:
            print(f"Error calculating data for molecules: {e} in step {molecule_counter}")
            romper_todo = True
            break
        ...
        try:
            results = []
            futures = [executor_gpu.submit(sample_step_graph, graph, tensor.clone(), probs_quadrupletas.detach(), all_smiles_molecule, idp, num_swaps[idp], molecule_counter-1) for graph, tensor, probs_quadrupletas, all_smiles_molecule, idp in zip(noisy_graphs, tensors_allmolecules, probs_quadrupletas_mod_list, all_smiles_for_all_molecules, range(len(num_swaps)))]
            ...
            if len(not_done) > 0:
                for f in not_done:
                    f.cancel()
                raise TimeoutError(f"Timed out after {future_timeout_seconds}s waiting for sample_step_graph; {len(not_done)} incomplete")

            # Get the results (all are done here)
            results = [future.result() for future in futures]
        except Exception as e:
            print(f"Error sampling next-step graphs: {e} in step {molecule_counter-1}")
            romper_todo = True
            break
    ...
    for i in range(len(num_swaps)):
        ...
        best_time = best_time_all_molecules[i]

        # Use the best molecule for further processing
        g_gen = components_to_graph(noisy_graphs[i].nodes(data=True), best_tensor)
        ...
        df_generated_temporal = df_generated_temporal._append({'smiles': smiles_str, 'molecular_formula': formula, 'num_atoms': num_atoms, 'time': best_time}, ignore_index=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir_suffix', type=str, default='251012_database_allmolecules_main_2_22_confps_timepred_2_22_confps_sinexplicit_unseed2', help='Suffix for the output directory name.')
    ...
    parser.add_argument('--batch_size_process', type=int, default=10, help='Batch size for processing molecules within process_batch.')
    parser.add_argument('--save_every_n_batches', type=int, default=10, help='Save cumulative results every N sampling batches.')
    parser.add_argument('--num_workers', type=int, default=8, help='Number of worker processes for parallel execution.')
    parser.add_argument('--future_timeout_minutes', type=float, default=5.0, help='Timeout in minutes for each pool of futures inside a step.')
   ...
   # read all smiles that pass the filters
    if args.input_smiles_csv.endswith('.smiles'):
        with open(args.input_smiles_csv, 'r') as inf:
            df = inf.readlines()
            # every smile has a \n at the end, so we remove it
            df = [smile.strip() for smile in df]
        ...
        with ProcessPoolExecutor(max_workers=args.num_workers) as executor_gpu: # Use argument value
            ...
            for num, conjunto in enumerate(train_dl):
                df_generated_batch_slice = process_batch(
                    ...
                    future_timeout_seconds=int(args.future_timeout_minutes * 60)
                )
