...
from lib_functions.models import GINEdgeQuadrupletPredictor_MorganFP
...
def process_batch(conjunto, model, num, b_molecule, cantidad):
    ...
    while molecule_counter < num_swaps_max:
        ...
        # if the molecule counter is 0, we initialize tensors and tracking
        if molecule_counter == 0:
            ...
            for i in range(len(num_swaps)):
                ...
                # initialize best tensor with the initial noisy tensor
                best_tensor_all_molecules[i] = tensors_allmolecules[i]
        
        # Compute theoretical time per molecule step, like in training
        time_predictions = []
        for i in range(len(num_swaps)):
            sigma_i = sigma_list[i]
            steps_for_molecule = max(1, num_swaps[i])
            progressed_steps = min(molecule_counter, steps_for_molecule)
            fraction = progressed_steps / steps_for_molecule
            nl = sigma_i * (1.0 - fraction)
            time_predictions.append(nl)
        ...
        # No time model used; noiselevel is theoretical time per molecule

        for i in range(len(num_swaps)):
            if molecule_counter == 0 or (molecule_counter < num_swaps[i]): 
                best_tensor_all_molecules[i] = tensors_allmolecules[i]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir_suffix', type=str, default='251010_database_allmolecules_main_2_22_confps_timepred_2_22_confps_sinexplicit_notime', help='Suffix for the output directory name.')
    ...
    try:
        ...
        print(f"Error: Could not find model checkpoint at: {args.model_checkpoint_path}")
        exit()
    except Exception as e:
        print(f"An error occurred while loading main model: {e}")
        exit()
    ...
    while True:  # Run indefinitely
        ...
        with ProcessPoolExecutor(max_workers=args.num_workers) as executor_gpu: # Use argument value
            ...
            for num, conjunto in enumerate(train_dl):
                df_generated_batch_slice = process_batch(conjunto, model, num, total_batches_processed, quantity) # Pass new arg
