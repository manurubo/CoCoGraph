...
from lib_functions.data_preparation_utils import calculate_2d_distances_ordered, embed_edges_with_cycle_sizes_norm, smiles_to_graph
...
def sample_step_graph(initial_graph, tensor, probs_quadrupletas_mod, all_smiles_molecule, idp, num_swaps, contador_molecula):
    if contador_molecula == 0:
        print(f"Comienza la molecule {all_smiles_molecule}", flush=True)
    ...
    while not roto and attempts < max_attempts:
            ...
            if error or (rompia % 5 == 0 and rompia >= 1) or (igual_molecula % 5 == 0 and igual_molecula >= 1):
                ...
                print(flat_prob_tensor.topk(5)) # si vuelve a petar, hay que mirar aqui que esta dando
                ...
                rompia = 0
                igual_molecula = 0
                #attempts += 1 # si vuelve a petar, activar esto
                ...
                if current_graph_molecule.has_edge(i1,j1) == False or current_graph_molecule.has_edge(i2,j2) == False:
                    ...
                    attempts += 1 # si ahora no se para, es porque era esto
                    continue
    ...
    if contador_molecula == 0:
        print(f"Termina la molecule {all_smiles_molecule}", flush=True)
...
def calculate_data_molecule_smiles(graph, tensor, num_swaps, current_swap, smile):
...
def process_batch(conjunto, model, num, b_molecule, cantidad, time_model):
    ...
    while contador_molecula < num_swaps_max:
        ...
        if contador_molecula == 0:
            futures = [executor_gpu.submit(calculate_data_molecule_smiles, graph, tensor, num_swaps, contador_molecula, smile) for graph, tensor, num_swaps, smile in zip(valid_graph_b, tensors_allmolecules, num_swaps, smiles)]
        else:
        ...
        # results = []
        # for graph,tensor,num_swaps,smile_unico in zip(valid_graph_b,tensors_allmolecules,num_swaps,smiles):
        #     result = calculate_data_molecule(graph,tensor,num_swaps,contador_molecula,smile_unico)
    ...
    for i in range(len(num_swaps)):
        ...
        #img.save(f'mols_gen/{str_date}/molecule_{b_molecule*1000+num*cantidad+i}_best.png')
...
if __name__ == "__main__":
    ...
    try:
        checkpoint = torch.load(f'models/241213_allmolecules_ffnet_fps_finetune/model_epoch_1_slice_22.pth')
        print("modelo_encontrado")
        checkpoint_time = torch.load(f'models/241216_all_timepred_fps_finetune/model_epoch_2_slice_22.pth')
    ...
    initial_range = 0
    cantidad = 50
    molecules_per_save = 50000  # 50K molecules per save (50 batches * 1000 molecules)
    total_batches_processed = 0
    df_generated_total = pd.DataFrame()
    while True:  # Run indefinitely
        # Randomly sample batch_size SMILES with replacement
        ...
        with ProcessPoolExecutor(max_workers=8) as executor_gpu:
            ...
            for num, conjunto in enumerate(train_dl):
                df_generated_batch_slice = process_batch(conjunto, model, num, total_batches_processed, cantidad, time_model)
        ...
        df_generated_batch.to_csv(f"mols_gen/{str_date}/generated_molecules_batch_{total_batches_processed}.csv", index=False)
        ...
        # Save total results every 50 batches (50K molecules)
        if total_batches_processed % 50 == 0:
